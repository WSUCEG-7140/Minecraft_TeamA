...
class Player:
    ...
    def __init__(self) -> None:
        ...
        #    v_t = v_0 + a * t
        # for the time at which you achieve maximum height, where a is 
        # the acceleration due to gravity and v_t = 0. This gives:
        #    t = - v_0 / a
        # Use t and the desired MAX_JUMP_HEIGHT to solve for v_0 (jump 
        # speed) in s = s_0 + v_0 * t + (a * t^2) / 2
    ...
    def update(self, dt: float, collision_checker: Callable[[tuple, int], tuple]) -> None:
        ...
        # collisions
        x, y, z = self.position
        self.position = collision_checker((x + dx, y + dy, z + dz), \
                                            self.PLAYER_HEIGHT)
    ...
    def check_player_within_world_boundaries(self) -> None:
        ...
        x = self.keep_player_within_coordinates(x , \
            boundary_size=WORLD_SIZE)
        z = self.keep_player_within_coordinates(z , \
            boundary_size=WORLD_SIZE)
        self.position = (x,y,z)
